...
fun EditExerciseInstructionsScreen(
    ...
    val enState = remember(sourceExercise?.id) {
        mutableStateOf(sourceExercise?.instructionsLocalized?.get("en")?.joinToString("\n") ?: "")
    }
    val ruState = remember(sourceExercise?.id) {
        mutableStateOf(sourceExercise?.instructionsLocalized?.get("ru")?.joinToString("\n") ?: "")
    }
    val beState = remember(sourceExercise?.id) {
        mutableStateOf(sourceExercise?.instructionsLocalized?.get("be")?.joinToString("\n") ?: "")
    }
    ...
    Scaffold(
                ...
                actions = {
                    IconButton(onClick = {
                        val updatedMap = mapOf(
                            "en" to enState.value.lines().map { it.trim() }.filter { it.isNotBlank() },
                            "ru" to ruState.value.lines().map { it.trim() }.filter { it.isNotBlank() },
                            "be" to beState.value.lines().map { it.trim() }.filter { it.isNotBlank() }
                        )

                        val isValid = updatedMap.values.all { it.isNotEmpty() }
                        ...
                        if (tempExercise?.id == exerciseId) {
                            viewModel.updateTempExercise { it.copy(instructionsLocalized = updatedMap) }
                        } else {
                            viewModel.updateExerciseInstructions(exerciseId, updatedMap)
                        }
        ...
        Column(
            ...
            Text(
                text = stringResource(R.string.start_each_instruction_with_a_verb),
                style = MaterialTheme.typography.bodyLarge,
                modifier = Modifier.padding(top = 8.dp)
            )
            Text(
                text = stringResource(R.string.use_one_line_per_step),
                style = MaterialTheme.typography.bodyLarge,
                modifier = Modifier.padding(top = 4.dp)
            )
            Text(
                text = stringResource(R.string.keep_instructions_concise_and_clear),
                style = MaterialTheme.typography.bodyLarge,
                modifier = Modifier.padding(top = 4.dp)
            )
            Text(
                text = stringResource(R.string.numbering_will_be_added_automatically),
                style = MaterialTheme.typography.bodyLarge,
            ...
            Spacer(
                modifier = Modifier.absolutePadding(bottom = 320.dp)
            )
        }
    }
}
